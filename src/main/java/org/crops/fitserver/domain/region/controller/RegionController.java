...
public class RegionController {
  ...
  public ResponseEntity<RegionDto> createRegion(@Valid @RequestBody CreateRegionRequest createRegionRequest) {
    return ResponseEntity.status(HttpStatus.CREATED).body(regionService.createRegion(createRegionRequest));
  }

  @PutMapping("/{regionId}")
  @PreAuthorize("hasRole('ROLE_ADMIN')")
  public ResponseEntity<RegionDto> updateRegion(@PathVariable Long regionId, @Valid @RequestBody UpdateRegionRequest updateRegionRequest) {
    return ResponseEntity.status(HttpStatus.NO_CONTENT).body(regionService.updateRegion(regionId, updateRegionRequest));
  }
