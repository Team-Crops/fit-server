...
if [ -n "$(docker images -a -q 728702143069.dkr.ecr.ap-northeast-2.amazonaws.com/fit-was --filter "before=728702143069.dkr.ecr.ap-northeast-2.amazonaws.com/fit-was:$ENV")" ]; then
  echo "[$TIME] > Remove WAS images not latest version." >> /home/ubuntu/app/deploy/deploy.log
  docker rmi -f $(docker images -a -q 728702143069.dkr.ecr.ap-northeast-2.amazonaws.com/fit-was --filter "before=728702143069.dkr.ecr.ap-northeast-2.amazonaws.com/fit-was:$ENV")
fi
