...
if [ -n "$(docker ps | grep fit-was-$TARGET_STATE)" ]; then
  echo "[$NOW_TIME] > Kill f-it WAS running with state port $TARGET_PORT in state $TARGET_STATE." >> /home/ubuntu/app/deploy/deploy.log
...
&& docker-compose -f /home/ubuntu/app/deploy/docker/docker-compose.was.yml up $TARGET_STATE \
